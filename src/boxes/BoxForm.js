...
export default ({ publish, box, afterCreate, authorize }) => {
  ...
  if (!afterCreate && box.updated !== updated) {
    update(fields, editing, box)
    if (editing.length === 0) {
      setUpdated(box.updated)
      setLoading(false)
    }
  }
  const onFocus = (key) => !afterCreate ? setEditing([...editing, key]) : null
  const onBlur = (e, key) => !afterCreate && (e.target.value === box.data[key]) ? setEditing(editing.filter((field) => field !== key)) : null
